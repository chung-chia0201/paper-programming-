...
## *****************************************************
##  DESCRIPTION :
##  This is the server-side(socket) on ReSpeaker Core v2.0, and the IP needs to be changed. it is send row data to PC.
##
#
...
class detect_freq(object):
    ...
    def listen(self):
        ...
        self.row_data=str(temp_buffer)
        self.row_data=self.row_data.replace(self.row_data[-1],"")
        self.row_data=self.row_data.replace(self.row_data[0],"")
    ...
    while True:
        ...
        while True:
            try:
                ...
                conn.send(self.row_data.encode())
            except Exception:
                continue
        conn.close()
